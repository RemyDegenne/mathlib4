...
theorem eq_condKernel_of_measure_eq_compProd' (κ : Kernel α Ω) [IsSFiniteKernel κ]
    [ρ.HasCondKernel] (hκ : ρ = ρ.fst ⊗ₘ κ) {s : Set Ω} (hs : MeasurableSet s) :
...
theorem eq_condKernel_of_measure_eq_compProd [CountableOrCountablyGenerated α Ω]
    (κ : Kernel α Ω) [IsFiniteKernel κ]
    [ρ.HasCondKernel] (hκ : ρ = ρ.fst ⊗ₘ κ) :
    ∀ᵐ x ∂ρ.fst, κ x = ρ.condKernel x := by
  have hκ' : ρ = ρ.fst ⊗ₘ ρ.condKernel := ρ.isCondKernel_condKernel.disintegrate.symm
  nth_rw 1 [hκ'] at hκ
  rw [Kernel.compProd_eq_iff] at hκ
  exact hκ.symm

lemma condKernel_compProd [CountableOrCountablyGenerated α Ω]
    (μ : Measure α) [IsFiniteMeasure μ] (κ : Kernel α Ω) [IsMarkovKernel κ] :
...
lemma Kernel.apply_eq_measure_condKernel_of_compProd_eq [CountableOrCountablyGenerated β Ω]
    {ρ : Kernel α (β × Ω)} [IsFiniteKernel ρ] [ρ.HasCondKernel]
    {κ : Kernel (α × β) Ω} [IsFiniteKernel κ]
    (hκ : Kernel.fst ρ ⊗ₖ κ = ρ) (a : α) :
    (fun b ↦ κ (a, b)) =ᵐ[ρ.fst a] (ρ a).condKernel := by
...
lemma Kernel.condKernel_apply_eq_condKernel [CountableOrCountablyGenerated α β]
    [CountableOrCountablyGenerated β Ω]
    (κ : Kernel α (β × Ω)) [IsFiniteKernel κ] [HasCondKernel κ] (a : α) :
    (fun b ↦ κ.condKernel (a, b)) =ᵐ[Kernel.fst κ a] (κ a).condKernel :=
  Kernel.apply_eq_measure_condKernel_of_compProd_eq κ.isCondKernel_condKernel.disintegrate a

lemma condKernel_const [CountableOrCountablyGenerated α β] [CountableOrCountablyGenerated β Ω]
    (ρ : Measure (β × Ω)) [IsFiniteMeasure ρ] [ρ.HasCondKernel] (a : α) :
    have := Kernel.hasCondKernel_const (ρ := ρ) (mα := mα)
...
theorem eq_condKernel_of_kernel_eq_compProd [CountableOrCountablyGenerated α β]
    [CountableOrCountablyGenerated β Ω]
    {ρ : Kernel α (β × Ω)} [IsFiniteKernel ρ] [ρ.HasCondKernel]
    {κ : Kernel (α × β) Ω} [IsFiniteKernel κ] (hκ : Kernel.fst ρ ⊗ₖ κ = ρ) (a : α) :
